<template>
  <div class="board-detail">
    <div class="common-buttons">
      <button type="button" class="w3-button w3-round w3-blue-gray" v-on:click="fnUpdate" v-show="(author === nowuser)">수정</button>&nbsp;
           <button type="button" class="w3-button w3-round w3-red" v-on:click="fnDelete" v-show="(author === nowuser)">삭제</button>&nbsp;
    ...
    <table class="w3-table-all">
          ...
          <tr v-for="(row, idx) in list" :key="idx">
            <td>{{ row.comment }}</td>
            <td>{{ row.id }}</td>
            <button type="button" class="w3-button w3-round w3-red" v-on:click="deleteComment(row.idx)" v-show="(row.id === nowuser)" >삭제</button>&nbsp;
          </tr>
          </tbody>
        </table>
        <div>
            <!-- 댓글입력창 -->
             <p class="comment-form-comment">
              ...
              <textarea cols="50" rows="2"  v-model="comment"
  ...
  methods: {
    ...
    fnDelete() {
     ...
     commentList() {
        ...
        addComment(){
          let apiUrl = this.$serverUrl + '/comment/'+ this.idx
          this.form = {
            "board": this.idx,
